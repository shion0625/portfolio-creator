...
type WorkLoaderConfig struct {
	...
	Fetch func(keys []string) ([][]*domain.Work, []error)
...
func (l *WorkLoader) Load(key string) ([]*domain.Work, error) {
...
func (l *WorkLoader) LoadThunk(key string) func() ([]*domain.Work, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() ([]*domain.Work, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *WorkLoader) LoadAll(keys []string) ([][]*domain.Work, []error) {
	results := make([]func() ([]*domain.Work, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	works := make([][]*domain.Work, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *WorkLoader) LoadAllThunk(keys []string) func() ([][]*domain.Work, []error) {
	results := make([]func() ([]*domain.Work, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *WorkLoader) Prime(key string, value []*domain.Work) bool {
	l.mu.Lock()
	var found bool
	if _, found = l.cache[key]; !found {
		// make a copy when writing to the cache, its easy to pass a pointer in from a loop var
		// and end up with the whole cache pointing to the same value.
		cpy := make([]*domain.Work, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
...
func (l *WorkLoader) unsafeSet(key string, value []*domain.Work) {
	if l.cache == nil {
		l.cache = map[string][]*domain.Work{}
	}
	l.cache[key] = value
}
